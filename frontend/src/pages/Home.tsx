      ...
      {view === 'main' && (
        <>
          <section className="relative border-b border-line-hairline pb-12 pt-4 sm:pb-16 lg:grid lg:grid-cols-[1fr_18rem] lg:gap-14">
            ...
            <aside className="mt-10 border-l-2 border-line-strong pl-5 lg:mt-1">
              ...
              <p className="mt-4 text-micro text-ink-muted">
                ...
                {dossier.model.execution === 'local_gpu_offline' ? 'Qwen local en GPU' : dossier.model.name} · 0
                llamadas de IA en esta web
              </p>
            </aside>
          </section>
          ...
          <section id="objetivos" className="mt-20">
            <div className="grid gap-8 lg:grid-cols-[1fr_20rem] lg:items-start">
              <div className="max-w-3xl">
                ...
                <h2 className="mt-3 text-title font-semibold text-ink-primary">
                  La reforma: seis ejes y treinta materias
                </h2>
                <p className="mt-4 text-body text-ink-secondary">
                  Empieza por los seis ejes principales y continúa, en esta misma sección, con el desglose completo del
                  PDF. Cada explicación fue aceptada sólo cuando su cita aparece en la página indicada.
                </p>
              </div>
            ...
            <h3 className="mt-10 text-lede font-semibold text-ink-primary">Vista rápida · 6 ejes</h3>
            ...
            <div id="lectura-completa" className="mt-14 border-t border-line-hairline pt-10">
              <div className="grid gap-8 lg:grid-cols-[1fr_20rem] lg:items-start">
                <div className="max-w-3xl">
                  ...
                  <h3 className="mt-3 text-lede font-semibold text-ink-primary">Lectura completa · 30 materias</h3>
                      ...
                      <div className="grid gap-px border-t border-line-hairline bg-line-hairline lg:grid-cols-2">
                        {topics.map((topic) => (
                          <article key={topic.id} className="bg-surface-raised p-5 sm:p-6">
                            ...
                            <p className="mt-3 text-caption text-ink-secondary">
                              <span className="font-semibold text-ink-primary">Efecto fiscal:</span>{' '}
                              {topic.fiscal_effect}
                            </p>
                            <details className="mt-4 border-t border-line-hairline pt-3">
                              <summary className="cursor-pointer text-caption font-semibold">
                                Supuestos, riesgos y cita
                              </summary>
                              <p className="mt-3 text-caption text-ink-secondary">
          ...
          <section id="medidores" className="mt-24 border-t border-line-hairline pt-12">
            <div className="max-w-3xl">
              <p className="text-micro font-semibold uppercase tracking-[0.18em] text-ink-muted">
                Medidores desarmables
              </p>
              <h2 className="mt-3 text-title font-semibold text-ink-primary">Dónde cae el debate</h2>
              <p className="mt-4 text-body text-ink-secondary">
                No son notas de bondad ni un detector mágico de sesgo. Cada aguja resume componentes nombrados; abre
                “Ver cálculo” para auditar qué la mueve. Las fotografías son neutrales y simétricas.
              </p>
            </div>
          ...
          <section id="debate" className="mt-24 border-t border-line-hairline pt-12">
            <div className="max-w-3xl">
              ...
              <p className="mt-4 text-body text-ink-secondary">
                Aleph responde por afirmación y reúne automáticamente las noticias citadas por cada posición. La página
                sigue siendo offline: una actualización del corpus recalcula esta lista antes del próximo deploy.
              </p>
            </div>
                ...
                >
                  ...
                  {debateNews(question, sourceById)[0] && (
                    <p className="mt-3 text-micro font-semibold uppercase tracking-wide text-ink-muted">
                      Debate público · {debateNews(question, sourceById).length} publicaciones incorporadas · última{' '}
                      {new Date(debateNews(question, sourceById)[0]!.published_at).toLocaleDateString('es-CL')}
                    </p>
                  )}
                  ...
                  {debateNews(question, sourceById).length > 0 && (
                    <details className="mt-5 border-t border-line-hairline pt-4">
                      <summary className="cursor-pointer text-caption font-semibold text-ink-primary">
                        Noticias que actualizan este debate
                      </summary>
                        ...
                        {debateNews(question, sourceById).map((source) => (
                          ...
                          >
                            <span className="block text-micro font-semibold uppercase text-ink-muted">
                              {source.publisher} · {new Date(source.published_at).toLocaleDateString('es-CL')}
                            </span>
          ...
          <section id="evidencia-comparada" className="mt-24 border-t border-line-hairline pt-12">
            <div className="max-w-3xl">
              <p className="text-micro font-semibold uppercase tracking-[0.18em] text-ink-muted">
                Teoría y evidencia global
              </p>
              <h2 className="mt-3 text-title font-semibold text-ink-primary">¿Qué sabemos fuera de este debate?</h2>
              <p className="mt-4 text-body text-ink-secondary">
                El modelo local contrastó cada mecanismo con evidencia empírica e institucional internacional. No
                traslada automáticamente un resultado extranjero a Chile: separa lo respaldado, lo condicional y lo que
                aún debe medirse.
              </p>
            </div>
            <div className="mt-8 grid gap-5 lg:grid-cols-2">
              {theory.topics.map((topic) => (
                <article key={topic.id} className="border border-line-hairline bg-surface-card p-6 sm:p-8">
                  ...
                  <p className="mt-5 text-caption text-ink-secondary">
                    <span className="font-semibold text-ink-primary">Aplicado a la reforma:</span>{' '}
            ...
            <p className="mt-5 text-micro text-ink-muted">
              Qwen local · {theory.model.usage.total_tokens.toLocaleString('es-CL')} tokens · salida JSON validada · 0
              llamadas de IA desde esta página.
            </p>
          </section>
        </>
      )}
      ...
      {view === 'actors' && (
        <>
          <section className="border-b border-line-hairline pb-12 pt-4 sm:pb-16">
            <p className="text-micro font-semibold uppercase tracking-[0.2em] text-ink-muted">
              Registro público y archivo
            </p>
            <h1 className="mt-5 max-w-4xl text-display font-semibold text-ink-primary">
              Actores, municipios y fuentes.
            </h1>
            <p className="mt-6 max-w-3xl text-lede text-ink-secondary">
              Quién intervino en el debate, qué sostuvo, qué trayectoria pública está verificada y dónde consultar cada
              publicación original. Esta información está separada del análisis factual de la reforma.
            </p>
            <dl className="mt-8 grid max-w-4xl grid-cols-2 gap-px bg-line-hairline sm:grid-cols-4">
              {[
                ['Actores', census.coverage.actors_indexed],
                ['Menciones verificadas', census.coverage.accepted_mentions],
                ['Capturas', sources.capture_count],
                ['Brechas', sources.gap_count],
              ].map(([label, value]) => (
                <div key={label} className="bg-surface-card p-4">
                  <dt className="text-micro uppercase text-ink-muted">{label}</dt>
                  <dd className="mt-2 text-title font-semibold tabular text-ink-primary">{value}</dd>
                </div>
              ))}
            </dl>
          </section>
          ...
          <section id="censo-actores" className="mt-24 border-t border-line-hairline pt-12">
            <div className="grid gap-8 lg:grid-cols-[1fr_20rem] lg:items-start">
              <div className="max-w-3xl">
                ...
                <h2 className="mt-3 text-title font-semibold text-ink-primary">
                  Todos los actores sustantivos encontrados
                </h2>
                <p className="mt-4 text-body text-ink-secondary">
                      ...
                      {group.map((actor) => (
                        <article key={`${actor.entity_kind}-${actor.id}`} className="p-5">
                          ...
                          <details className="mt-3">
                            <summary className="cursor-pointer text-caption font-semibold">
                              {actor.source_ids.length} {actor.source_ids.length === 1 ? 'fuente' : 'fuentes'} · ver
                              evidencia
                            </summary>
          ...
          <section id="actores" className="mt-24 border-t border-line-hairline pt-12">
            <div className="max-w-3xl">
              ...
              <p className="mt-4 text-body text-ink-secondary">
                Pasa el cursor o enfoca un nombre en el debate para ver su ficha. Los antecedentes se muestran aparte:
                nunca cambian un veredicto factual.
              </p>
            </div>
            <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {dossier.actors.map((actor) => (
                <article key={actor.id} className="border border-line-hairline bg-surface-card p-4">
                  ...
                  <details className="mt-4 border-t border-line-hairline pt-3">
                    <summary className="cursor-pointer text-caption font-semibold">
                      Trayectoria y acciones verificadas
                    </summary>
          ...
          <section id="municipios" className="mt-24 border-t border-line-hairline pt-12">
            <div className="grid gap-8 lg:grid-cols-[1fr_20rem] lg:items-start">
              ...
              <aside className="border-l-2 border-line-strong pl-5 text-caption text-ink-secondary">
                <p className="font-semibold text-ink-primary">
                  {municipal.coverage.actors_indexed} alcaldes/as · {municipal.coverage.municipal_sources_curated}{' '}
                  fuentes
                </p>
